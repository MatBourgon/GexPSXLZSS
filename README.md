This program can decompress files stored in Gex 2: Enter the Gecko's BIGFILE.dat, either from pre-extracted files stored on disk, or while they are still stored in the BIGFILE (in-memory, additional parsing of the BIGFILE is required). It can additionally compress files using the same algorithm, creating files that can be decompressed using the same algorithm, while also being more space efficient.
...
### Note:
This program cannot decompress and recompress the entire BIGFILE.dat file itself. The BIGFILE.dat format is separate from this compression algorithm (typically: a header and then all the compressed files). In this case, you would need to parse the header yourself and decompress/recompress the data as you see fit. An example of decompressing a file can be seen in `src/GexLZSSProgram.cpp`
